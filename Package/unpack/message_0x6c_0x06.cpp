/*
 * message_0x6C_0x06.cpp
 *
 *  Created on: 2016-10-18
 *      Author: root
 */

#include "message_0x6c_0x06.h"

using namespace Package;
using namespace Package::unpack;

void unpack_0x6C_0x06_message(char*& in_data, size_t& in_data_size, node*& data){
...
message_0x6C_0x06::message_0x6C_0x06(log_handler*& log) {
	// TODO Auto-generated constructor stub
	logger_= log;
}

message_0x6C_0x06::~message_0x6C_0x06() {
	// TODO Auto-generated destructor stub
}

int message_0x6C_0x06::handler(char*& in_data, size_t& in_data_size, net_layer*& in_net,  node*& data){
	...
	if(check_stream(in_data, in_data_size, is_to_next, err) == 0){
		...
		//		"strength":""       					// 接收场强
		// }
		unpack_0x6C_0x06_message(in_data, in_data_size, data);

		// Write Log
		if(logger_){
			string_append info;
			info.add("unpack 0x6C_0x06 success!");
		...
		if(!is_to_next){
			...
			if(logger_){
				string_append info;
				info.add("unpack 0x6C_0x06 package fail! err_msg:")->add(err.c_str());
...
int message_0x6C_0x06::check_stream(char*& in_data, size_t& in_data_size, bool& is_to_next, string& err){
	char comp[2] = {0x6c, 0x06};
